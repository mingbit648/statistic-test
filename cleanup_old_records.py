...
def cleanup_old_records():
    ...
    for i, record in enumerate(to_delete, 1):
        ...
        print(f"\n[{i}/{len(to_delete)}] 删除: {record_name} (id={record_id})")
        ...
        if standard_path:
            ...
            if not is_used_by_others:
                ...
                print(f"  ℹ️  标准文件被其他记录使用，保留: {standard_path}")
    ...
    print("\n⚠️  重要: 新建的记录ID会从 {0} 开始\n".format(max([item["id"] for item in to_keep] or [0]) + 1))
...
def verify_data_integrity():
    ...
    for record in history:
        ...
        if record_path and not os.path.exists(record_path):
            if not os.path.exists(os.path.join(UPLOADS_DIR, record_path)):
                missing_files.append((record["id"], record_path))
    
    if missing_files:
        print(f"⚠️  发现 {len(missing_files)} 个缺失文件:")
        for record_id, path in missing_files:
            print(f"  - id={record_id}: {path}")
    else:
        print(f"✅ 文件完整性检查通过")
